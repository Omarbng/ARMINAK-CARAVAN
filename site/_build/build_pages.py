...
<section class="hero" id="hero">
  <div class="hero__media">
    <div class="hero__layer">
      <!-- The mp4 is authored with a crossfaded seam, so native `loop` runs it
           unbroken with no jump cut. preload="none" and no autoplay attribute:
           main.js calls play() only when the film will actually be shown, so
           poster routes (mobile, reduced motion) never fetch it. -->
  ...
  <button class="hero__playback" id="heroPlayback" type="button" aria-pressed="true">
    <span class="hero__playback-label" data-i18n="hero.pause">Pause</span>
    <span class="hero__playback-icon">
      <svg class="i-pause" viewBox="0 0 12 12" aria-hidden="true"><rect x="2" y="1.5" width="2.8" height="9" rx="0.7"/><rect x="7.2" y="1.5" width="2.8" height="9" rx="0.7"/></svg>
      <svg class="i-play" viewBox="0 0 12 12" aria-hidden="true"><path d="M3 1.6v8.8l7-4.4z"/></svg>
      <svg class="hero__playback-ring" viewBox="0 0 44 44" aria-hidden="true">
        <circle id="heroRing" cx="22" cy="22" r="20.4" stroke-dasharray="128.2" stroke-dashoffset="128.2"/>
      </svg>
    </span>
  </button>
</section>
